<script lang="ts">
	...
	// Navigation via URL — each level pushes to history
	function goProjects() { goto(buildUrl({})); }
	function goProject(p: ProjectInsight) { goto(buildUrl({ project: p.project })); }
	function goDir(p: ProjectInsight, d: DirectoryInsight) { goto(buildUrl({ project: p.project, dir: d.dir })); }
	function goFile(p: ProjectInsight, d: DirectoryInsight, f: FileInsight) { goto(buildUrl({ project: p.project, dir: d.dir, file: f.path })); }
...
<div class="px-4 py-4 sm:px-6 sm:py-5">
	...
	<!-- Recent sessions -->
	{#if a.recentSessions.length > 0 && view.level === 'projects'}
		<div class="mb-6">
